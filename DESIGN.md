...
**Version 0.1 · locked 2026-08-25**

**Status:** accepted 2026-08-25 (the operator). Product name Envoy. Home `bulletin/envoy/`.
...
It holds membership (`MAP.md` librarian), published status (vault sitreps), and mail notes (bells for inbox letters). Local files stay the record for the map, `NOW.md`, local `STATUS.md`, and inbox letters. Status is not a conversation. Mail is.
...
`services` is the opt-in list. First two values: `status`, `mail`. Absent list means not subscribed. A later type is another entry, not a change to these two.
...
- Owner-roots path move (`STATUS.md` → `_status/`, `inbox/` → `_inbox/`)
